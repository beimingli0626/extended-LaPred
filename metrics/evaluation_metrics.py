...
class EvaluationMetrics(Metric):
    ...
    def compute(self, model_outputs, data):
        ...
        :return: computed ADE and FDE loss, noticed that they are summed across the minibatch, 
                they are instead averaged outside when aggregating, checkout trainer.py aggregate_metrics for detail
        """
        ...
        ade_1 = err[:, 0].mean(-1).sum()                                    # Average pointwise L2 distances accross points. 
                                                                            # Note that we sort the modality by likelihood in MTP, err[:, 0] corresponds to most likely modality
        fde_1 = err[:, 0, -1].sum()                                         # Average distances between final points of prediction and ground truth
        ...
        ade_5 = err[:, :5][row_idcs, ade_min_idcs_5].mean(-1).sum()         # Take average about points, then sum over minibatch         
        fde_5 = err[:, :5][row_idcs, fde_min_idcs_5][:, -1].sum()           # (batch_size, 2*pred_size) -> (batch_size, 1) -> (1)
        ...
        ade_10 = err[row_idcs, ade_min_idcs_10].mean(-1).sum()              # Take average about points, then sum over minibatch
        fde_10 = err[row_idcs, fde_min_idcs_10][:, -1].sum()                # (batch_size, 2*pred_size) -> (batch_size, 1) -> (1)

        return {'ade_1':ade_1, 'fde_1':fde_1, 'ade_5':ade_5, 'fde_5':fde_5, 'ade_10':ade_10, 'fde_10':fde_10, 'data_size':preds.shape[0]}
